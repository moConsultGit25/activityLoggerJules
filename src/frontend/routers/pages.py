...
@router.get("/auth/m365/initiate", name="initiate_m365_auth_frontend")
async def initiate_m365_auth_frontend(request: Request):
    ...
    backend_api_authorize_url = str(request.url_for('m365_authorize_redirect'))
...
async def handle_m365_disconnect(request: Request):
    ...
    api_disconnect_url = str(request.url_for('m365_disconnect'))
...
# --- Google Gmail OAuth Frontend Routes ---
@router.get("/auth/google/initiate", name="initiate_google_auth_frontend")
async def initiate_google_auth_frontend(request: Request):
    if not request.state.user_authenticated:
        return RedirectResponse(url=request.url_for('serve_login_page') + "?error_message=Please log in to connect Google.", status_code=status.HTTP_302_FOUND)
    backend_api_authorize_url = str(request.url_for('google_authorize_user_redirect'))
    return RedirectResponse(url=backend_api_authorize_url, status_code=status.HTTP_302_FOUND)

@router.post("/auth/google/disconnect", name="handle_google_disconnect")
async def handle_google_disconnect(request: Request):
    if not request.state.user_authenticated:
        return RedirectResponse(url=request.url_for('serve_login_page') + "?error_message=Please log in.", status_code=status.HTTP_302_FOUND)
    token = get_auth_token_from_cookie(request)
    if not token:
    ...
    api_disconnect_url = str(request.url_for('google_disconnect')) # Correct API route name
    headers = {"Authorization": f"Bearer {token}"}
    message, error = None, None
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(api_disconnect_url, headers=headers)
            response.raise_for_status()
            message = response.json().get("message", "Google account disconnected successfully.")
        except httpx.HTTPStatusError as e:
            error = e.response.json().get("detail", "Failed to disconnect Google account.")
        except httpx.RequestError:
            error = "Google disconnect service unavailable."
...
async def serve_dashboard_page(request: Request, page: int = Query(1, ge=1), message: Optional[str] = None, error: Optional[str] = None):
    ...
    logs_data, api_error_logs = None, error
    m365_status_data: Dict[str, Any] = {"is_connected": False, "account_email": None, "error": None}
    google_status_data: Dict[str, Any] = {"is_connected": False, "account_email": None, "error": None}

    async with httpx.AsyncClient() as client:
        headers = {"Authorization": f"Bearer {token}"}
        # 1. Fetch M365 connection status
        try:
            status_response_m365 = await client.get(str(request.url_for('get_m365_connection_status')), headers=headers)
            status_response_m365.raise_for_status()
            m365_status_data = status_response_m365.json()
        except httpx.HTTPStatusError as e:
            if e.response.status_code == 401: return RedirectResponse(url=request.url_for('serve_login_page') + "?error_message=Session expired.", status_code=status.HTTP_302_FOUND)
            m365_status_data["error"] = f"M365 Status: {e.response.json().get('detail', e.response.status_code)}"
        except httpx.RequestError: m365_status_data["error"] = "M365 status service unavailable."

        # 2. Fetch Google connection status
        try:
            google_status_api_url = str(request.url_for('get_google_connection_status')) # Correct API route name
            status_response_google = await client.get(google_status_api_url, headers=headers)
            status_response_google.raise_for_status()
            google_status_data = status_response_google.json()
        except httpx.HTTPStatusError as e:
            if e.response.status_code == 401: return RedirectResponse(url=request.url_for('serve_login_page') + "?error_message=Session expired.", status_code=status.HTTP_302_FOUND)
            google_status_data["error"] = f"Google Status: {e.response.json().get('detail', e.response.status_code)}"
        except httpx.RequestError: google_status_data["error"] = "Google status service unavailable."

        # 3. Fetch Activity Logs
        try:
            params = {"page": page, "page_size": 5}
            logs_api_response = await client.get(str(request.url_for('get_activity_logs')), headers=headers, params=params)
            logs_api_response.raise_for_status()
            logs_data = logs_api_response.json()
        except httpx.HTTPStatusError as e:
            if e.response.status_code == 401: return RedirectResponse(url=request.url_for('serve_login_page') + "?error_message=Session expired.", status_code=status.HTTP_302_FOUND)
            api_error_logs = f"Logs: {e.response.json().get('detail', e.response.status_code)}"
        except httpx.RequestError: api_error_logs = "Activity logs service unavailable."

    final_error_message = api_error_logs or m365_status_data.get("error") or google_status_data.get("error")

    return templates.TemplateResponse("dashboard.html", {
        "request": request, "title": "Dashboard", "logs_data": logs_data,
        "error_message": final_error_message, "success_message": message,
        "m365_connection_status": m365_status_data,
        "google_connection_status": google_status_data
    })

# --- EML Upload & Cloud Sync Handlers (as before) ---
...
async def handle_eml_upload_form(request: Request, eml_file: Annotated[UploadFile, File(...)]):
    if not request.state.user_authenticated: return RedirectResponse(url=request.url_for('serve_login_page') + "?error_message=Login required.", status_code=status.HTTP_302_FOUND)
    token = get_auth_token_from_cookie(request)
    if not token: return RedirectResponse(url=request.url_for('serve_login_page') + "?error_message=Token missing.", status_code=status.HTTP_302_FOUND)
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            response.raise_for_status(); data = response.json(); task_id = data.get('task_id',''); msg = data.get("message","Processed.")
            redirect_url = f"{request.url_for('serve_dashboard_page')}?message={msg}&submitted_task_id={task_id}"
        except httpx.HTTPStatusError as e: redirect_url = f"{request.url_for('serve_dashboard_page')}?error={e.response.json().get('detail','EML processing failed.')}"
        except httpx.RequestError: redirect_url = f"{request.url_for('serve_dashboard_page')}?error=EML upload service unavailable."
        finally: await eml_file.close()
...
async def handle_cloud_sync_trigger(request: Request, max_emails: int = Form(10), mark_as_read: bool = Form(True)):
    ...
    async with httpx.AsyncClient(timeout=120.0) as client:
        try:
            ...
            response.raise_for_status(); data = response.json(); task_id = data.get('task_id',''); msg = data.get("message","Cloud sync initiated.")
            redirect_url = f"{request.url_for('serve_dashboard_page')}?message={msg}&submitted_task_id={task_id}"
        except httpx.HTTPStatusError as e: redirect_url = f"{request.url_for('serve_dashboard_page')}?error={e.response.json().get('detail','Cloud sync failed.')}"
        except httpx.RequestError: redirect_url = f"{request.url_for('serve_dashboard_page')}?error=Cloud sync service unavailable."
    return RedirectResponse(url=redirect_url, status_code=status.HTTP_302_FOUND)
